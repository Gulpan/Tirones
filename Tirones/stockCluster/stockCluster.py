...
class stockCluster:
    ...
    def makeCluster(self, keyString='variation'):
        ...
        names = [company.name for company in self.stockList]
        ...
        for i in range(n_labels + 1):
            print('Cluster %i: %s' % ((i + 1), ', '.join(names[self.labels == i])))
    ...
    def plotCluster(self):
        ...
        for index, (name, label, (x, y)) in enumerate(
                zip(names, self.labels, embedding.T)):
